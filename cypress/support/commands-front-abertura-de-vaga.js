...
Cypress.Commands.add(
  "preenche_formulario_de_vaga",
  (observacao) => {    
    cy.get(locator.CADASTRA_VAGA.click_no_caledario).click();
    cy.get(locator.CADASTRA_VAGA.clcik_inicio_imediato).contains('Início imediato').click();    
    cy.get('[label="Perfil profissional"] input').click()
    cy.get('#options-wrapper ul li').then(($perfil)=>{                  
      const retorno  = dados_random.dados_random($perfil)
